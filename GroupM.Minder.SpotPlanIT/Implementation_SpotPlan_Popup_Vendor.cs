...
{
    public partial class Implementation_SpotPlan_Popup_Vendor : Form
    {
        ...
        DBManager m_db;
        public Implementation_SpotPlan_Popup_Vendor(string strMedia)
        {
            InitializeComponent();
            m_value = strMedia;
            m_db = new DBManager();
        ...
        private void Implementation_SpotPlan_Popup_Vendor_Load(object sender, EventArgs e)
        {
            gvDetail.AutoGenerateColumns = false;
            dtMaster = m_db.SelectVendorByMedia(m_value);
        ...
        private void txtClient_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                DataRow[] rowsToCopy = dtMaster.Select(string.Format("Media_Vendor_ID like '%{0}%' OR Short_Name like '%{0}%'", txtClient.Text.Replace("'", "")));
