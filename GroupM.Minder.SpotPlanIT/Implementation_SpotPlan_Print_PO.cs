...
namespace GroupM.Minder.SpotPlanIT
{
    public partial class Implementation_SpotPlan_Print_PO : Form
    {
        ...
        public Implementation_SpotPlan_Print_PO(string BB, string version, string startDate, string endDate, string template, string strMt, string strMst)
        {
            InitializeComponent();
            m_db = new DBManager();
        ...
        public void PrintPO(string Template, string Parameter, string BuyingBriefID, string Version, string StartDate, string EndDate, string SelectVendor, string mst)
        {
            ...
            try
            {
                ...
                if (SelectVendor != "")
                {
                    dtVendor = m_db.SelectVendorPO(SelectVendor);
                }
                ...
                for (int i = 0; i < dtVendor.Rows.Count; i++)
                {
                    ...
                    if (dtVendor.Rows[i]["PO_Media"].ToString() == "True")
                    {
                        ...
                        for (int j = 0; j < dtMediaCategory.Rows.Count; j++)
                        {
                            ...
                            if (mst == "FC")
                            {
                                ExcelUtil.ExcelSetValueString(workSheet, "A", 7, "FINECAST PLACEMENT ORDER");
                            }
                            else
                            {
                                ExcelUtil.ExcelSetValueString(workSheet, "A", 7, "CONNECTED TV PLACEMENT ORDER");
                            }
                            ...
                            {
                                noData = false;
                                ExcelUtil.ExcelSetValueStringFullTable(workSheet, 18, 1, 2, dtDetails);
                            }
                            ...
                            if (printFooter)
                            {
                                ...
                                {
                                    rowFooter += dtDetails.Rows.Count - 2;
                                }
        ...
        public void PrintPO_IT(string Template, string Parameter, string BuyingBriefID, string Version, string StartDate, string EndDate, string SelectVendor, string mt)
        {
            this.Cursor = Cursors.WaitCursor;
            try
            {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (mtit == true)
                {
                    //Master Media Type Online
                    PrintPO_IT(m_strPOExportPath, m_strParameterPath, m_strBuyingBriefID, m_strVersion, sd, ed, "", MediaTypeCode);
                }
                else
                {
                    //Test only FC and Connected TV
                    PrintPO(m_strPOExportPath, m_strParameterPath, m_strBuyingBriefID, m_strVersion, sd, ed, "", MediaSubTypeCode);
                }
            }
        }
