...
namespace MProprietary
{
    public partial class frmVendorList : Form
    {
        ...
        {
            dtGvdetail = connect.SelectVendorList();
        ...
        private void AddSelectData()
        {
            ...
            {
                MessageBox.Show("Vendor not found! ", "MProprietary", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                if (gvDetail.SelectedRows.Count == 0)
                {
                    MessageBox.Show("Please select vendor! ", "MProprietary", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
        ...
        private void txtSearch_KeyUp(object sender, KeyEventArgs e)
        {
            if (dtGvdetail.Rows.Count > 0)
            {
                ...
                string strCondition = string.Format("Media_Vendor_ID like '%{0}%' or Short_Name like '%{0}%'", strKeyWord);
