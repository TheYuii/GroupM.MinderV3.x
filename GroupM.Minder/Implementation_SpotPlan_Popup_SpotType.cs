...
{
    public partial class Implementation_SpotPlan_Popup_SpotType : Form
    {
        ...
        DBManager m_db;
        public Implementation_SpotPlan_Popup_SpotType()
        {
            InitializeComponent();
            m_db = new DBManager();
        ...
        private void Implementation_SpotPlan_Popup_SpotType_Load(object sender, EventArgs e)
        {
            gvDetail.AutoGenerateColumns = false;
            dtMaster = m_db.SelectSpotType();
        ...
        private void txtClient_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                DataRow[] rowsToCopy = dtMaster.Select(string.Format("SpotType like '%{0}%'", txtClient.Text.Replace("'", "")));
