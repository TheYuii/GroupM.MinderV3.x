...
{
    public partial class Utility_SearchPopup_CodeName3Col : Form
    {
        ...
        DBManager m_db;
        public Utility_SearchPopup_CodeName3Col(string strCode, string strName, string strCol, string strColName, string strTable, bool bIncludeInactive, string strUsername, string strFilter)
        {
            InitializeComponent();
            MasterCode = strCode;
            MasterName = strName;
            MasterColumn = strCol;
            MasterColumnName = strColName;
        ...
        private void Utility_SearchPopup_CodeName3Col_Load(object sender, EventArgs e)
        {
            gvDetail.AutoGenerateColumns = false;
            dtMaster = m_db.SelectMasterCommon3Col(MasterCode, MasterName, MasterColumn, MasterTable, IncludeInactive, Username, Filter);
            gvDetail.DataSource = dtMaster;
            gvDetail.Columns["Col"].HeaderText = MasterColumnName;
        }
        ...
        private void txtClient_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                DataRow[] rowsToCopy = dtMaster.Select(string.Format("Master like '%{0}%' OR MasterCode like '%{0}%' OR MasterName like '%{0}%'", txtClient.Text.Replace("'", "")));
