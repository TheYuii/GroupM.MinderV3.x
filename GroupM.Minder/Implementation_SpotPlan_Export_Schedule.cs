...
{
    public partial class Implementation_SpotPlan_Export_Schedule : Form
    {
        ...
        DBManager m_db;
        public Implementation_SpotPlan_Export_Schedule(string strCode, string strName, string strTable, bool bIncludeInactive, string strUsername, string strFilter)
        {
            InitializeComponent();
            MasterCode = strCode;
            MasterName = strName;
        ...
        private void Implementation_SpotPlan_Export_Schedule_Load(object sender, EventArgs e)
        {
            gvDetail.AutoGenerateColumns = false;
            dtMaster = m_db.SelectMasterCommon(MasterCode, MasterName, MasterTable, IncludeInactive, Username, Filter);
        ...
        private void txtClient_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                DataRow[] rowsToCopy = dtMaster.Select(string.Format("MasterName like '%{0}%' OR MasterCode like '%{0}%'", txtClient.Text.Replace("'", "")));
