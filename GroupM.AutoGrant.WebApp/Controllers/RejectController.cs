...
namespace GroupM.AutoGrant.WebApp.Controllers
{
    public class RejectController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Index(string Token, string RequestNo, string ApproverID, string ApproverUserID, string DelegateUserID, string Reason, string btnCheck)
        {
            try
            {
                if (btnCheck == "Submit")
